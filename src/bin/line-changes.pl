...
open my $fh, ">:encoding(utf8)", "line_modifications.csv"
  or die "line_modifications.csv: $!";
$csv->say(
    $fh,
    [
        "File",
...
our (
    $opt_c, $opt_d, $opt_D, $opt_e, $opt_E, $opt_g,
    $opt_h, $opt_l, $opt_q, $opt_s, $opt_t
);
...
for ( ; ; ) {
    if ( $state eq 'commit' ) {
        ...
        hide_escaped_quotes();
        bail_out('Expecting a diff command')
          unless ( m/^diff --git a\/([^ ]*) b\/(.*)/
            || m/^diff --git "a\/((?:[^"\\]|\\.)*)" "b\/((?:[^"\\]|\\.)*)"/
            || m/^diff --git a\/([^ ]*) "b\/((?:[^"\\]|\\.)*)"/
            || m/^diff --git "a\/((?:[^"\\]|\\.)*)" b\/(.*)/
            || m/^diff --git a\/(.*) b\/(.*)/ );
        ...
        while (<>) {
            ...
            if (/^--- /) {
                ...
                $loc += $#{ $flt{$from} } + 1
                  if ( $opt_g && output_source_code($to) );
                $nref = [ @{ $flt{$old} } ];
                $binary{$to} = 1 if ( $binary{$from} );
            }
            elsif (/^commit /) {
...
sub process_blocks {
    ...
    if ( @deleted_block && @added_block ) {
        ...
        my $min =
            scalar @deleted_block < scalar @added_block
          ? scalar @deleted_block
          : scalar @added_block;
        for ( my $i = 0 ; $i < $min ; $i++ ) {
            my $deleted_line = $deleted_block[$i];
            my $added_line   = $added_block[$i];
            process_line_change(
                $deleted_line->{file},    $deleted_line->{line_number},
                $deleted_line->{content}, 'modify',
                $added_line->{content},   $added_line->{line_number}
            );
        }
        ...
        if ( scalar @added_block > $min ) {
            for ( my $i = $min ; $i < scalar @added_block ; $i++ ) {
                ...
                process_line_change(
                    ...
                    $added_line->{content},
                    'add', undef, $added_line->{line_number}
                );
            }
        }
    }
...
sub process_line_change {
    my ( $file, $line_number, $content, $action, $new_content,
        $new_line_number )
      = @_;
    ...
    if ( $action eq 'delete' ) {
        ...
        if ( exists $line_modifications{$new_key} ) {
            ...
            $line_modifications{$new_key}{line_numbers}{$hash} =
              $new_line_number;
            $line_modifications{$new_key}{last_line_number} = $new_line_number;
            $line_modifications{$new_key}{content} =
...
sub push_to_cc {
    print "op=$op $old $new\n" if ($debug_push_cc);
    return                     if ( $op eq 'del' );
    push( @cc, { op => 'set', path => $old, lines => $oref } )
      if ( $oref != $nref && $op ne 'copy' );
...
sub output_source_code {
    ...
    my ($name) = @_;
    return ( $name =~
m/\.(C|c|cc|cpp|cs|cxx|hh|hpp|h\+\+|c\+\+|h|H|hxx|java|((php[3457s]?)|pht|php-s)|py)$/
    );
}
...
sub line_details {
    ...
    my $array      = () = $l =~ /\[/g;
    my $logical    = () = $l =~
/\=\=|[^>]\>\=|[^<]\<\=|\!\=|[^<]\<[^<]|[^->]\>[^>]|\!|\|\||\&\&|\bor\b|\band\b|\bnot\b|\bis\b/g;
    return
"$len $startspace $string $comment $comma $bracket $access $assignment $scope $array $logical";
}
...
sub output_line_modifications {
    for my $key ( sort keys %line_modifications ) {
        ...
        print
"File: $file, Modifications: $count, Status: $status, Line Number: $line_number, Content: $content\n";
    }
}
